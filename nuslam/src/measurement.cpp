...
std::vector<circle> clustering::circle_detection(std::vector<cluster> found_clusters){
    
    int size = found_clusters.size();
    ...
    for (unsigned long int i=0; i < found_clusters.size(); i++){
        double cluster_size = found_clusters[i].pt.size();
        if (cluster_size>3 && ((found_clusters[i].pt[0].x > 0)||(found_clusters[i].pt[0].y > 0))){
            ...
            for (unsigned long int j=0; j < cluster_size; j++){
                sum_x += (found_clusters[i].pt[j].x);
                sum_y += (found_clusters[i].pt[j].y);
            }
            
            centroids[i] = {.x = sum_x/cluster_size, .y = sum_y/cluster_size};
            arma::mat Z (cluster_size,4, arma::fill::zeros);
            for (unsigned long int j=0; j < cluster_size; j++){
                found_clusters[i].pt[j].x -= (centroids[i].x);
                found_clusters[i].pt[j].y -= (centroids[i].y);    
                double x = found_clusters[i].pt[j].x;
                double y = found_clusters[i].pt[j].y;
...
std::vector<circle> clustering::circle_classification(std::vector<cluster> found_clusters, std::vector<circle> circles){
    confirmed_circles.clear();
    ...
    for (unsigned long int i=0; i < found_clusters.size(); i++){
        ...
        if (cluster_size>3){
            if (circles.at(i).R2 < (obj_radius * obj_radius)*1.2){
                std::vector<double> angles;
                angles.resize(cluster_size - 2); 
                double angle_sum = 0;
                point p1 = found_clusters.at(i).pt.at(i);
                point p2 = found_clusters.at(i).pt.at(cluster_size - 1);
                ...
                for (unsigned long int j=1; j < cluster_size - 1; j++){
                    ...
                    ROS_WARN("Point 3: (%3.2f,%3.2f)", p3.x, p3.y);
                ...
                double mean_angle = angle_sum/(found_clusters[i].pt.size() - 2);
